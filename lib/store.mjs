...
export async function updateMonopoly(c,fn){if(demo)return serial(async()=>{const s=await load(),d=structuredClone(s.monopolies[c]||null),result=fn(d);s.monopolies[c]=d;await persist();return {game:d,result};});const ref=database().ref(`monopolies/${c}`);const snapshot=await ref.get();if(!snapshot.exists())throw new GameError('ไม่พบห้องนี้',404);let result;const out=await ref.transaction(d=>{d ||= structuredClone(snapshot.val());result=fn(d);return d;},undefined,false);if(!out.committed)throw new GameError('กรุณาลองอีกครั้ง',409);return {game:out.snapshot.val(),result};}
