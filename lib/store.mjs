...
async function load() { if (!memory) { try { memory=JSON.parse(await readFile(file,'utf8')); } catch(e) { if(e.code!=='ENOENT')throw e; memory={rooms:{},index:{},limits:{}}; } } return memory; }
...
export async function updateRoom(code,fn) {
  ...
  if(!out.committed)throw new GameError('ห้องไม่พร้อม กรุณาลองอีกครั้ง',409);
  return {room:out.snapshot.val(),result};
}
