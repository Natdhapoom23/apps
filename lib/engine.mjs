...
export function newRoom(code, now) {
  return { code, title: 'AI Workshop Live', createdAt: now, updatedAt: now, phase: 'lobby', accepting: true,
    ...
    settings: { base: 1000, speed: 500, steady: 100, showDistribution: false, capacity: 100 }, revision: 0 };
}
  ...
  if (action === 'join') {
    ...
    if (action === 'save') {
      ...
      room.settings = { base:integer(s.base,0,10000,'คะแนนถูก'), speed:integer(s.speed,0,10000,'โบนัสเร็ว'), steady:integer(s.steady,0,10000,'โบนัสไม่เปลี่ยนใจ'),
        capacity:integer(s.capacity,1,200,'จำนวนผู้เล่น'), showDistribution:!!s.showDistribution };
      ...
      const q = room.questions.find(q=>q.id === data.questionId); assert(q,'ไม่พบคำถาม');
      ...
      room.rounds[rid] = { id:rid, number:Object.keys(room.rounds).length+1, question:structuredClone(q), settings:structuredClone(room.settings), startAt:now, deadline:now+q.seconds*1000, answers:{}, revealed:false };
