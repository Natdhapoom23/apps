...
export function newRoom(code, now) {
  return { code, title: 'AI Workshop Live', createdAt: now, updatedAt: now, phase: 'lobby', accepting: true,
    groups: ['กลุ่ม 1','กลุ่ม 2','กลุ่ม 3','กลุ่ม 4','กลุ่ม 5'], questions: structuredClone(seedQuestions), players: {}, rounds: {}, current: null,
    settings: { base: 100, speed: 50, steady: 50, showDistribution: false, capacity: 100, shuffleQuestions: false }, revision: 0 };
}
...
export function mutate(room, action, data, actor, now) {
  ...
  if (action === 'join') {
    ...
    if (action === 'reset') {
      assert(room.phase !== 'open' && room.phase !== 'closed', 'ปิดและเฉลยรอบปัจจุบันก่อนเคลียร์ห้อง', 409);
      room.players = {}; room.rounds = {}; room.current = null; room.phase = 'lobby'; room.accepting = true;
      result.reset = true;
    } else if (action === 'save') {
