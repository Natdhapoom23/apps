...
import { configured, demo, readRoom, readPublic, listRooms, createRoom, deleteRoom, updateRoom, takeLimit, checkRules, getAdminPassword, setAdminPassword, listBoards, createBoard, readBoard, updateBoard, deleteBoard, listMonopolies, createMonopoly, readMonopoly, updateMonopoly, deleteMonopoly, listWheels, createWheel, readWheel, updateWheel, deleteWheel } from '../lib/store.mjs';
...
import { newMonopoly, monopolyPublic, mutateMonopoly, normalize } from '../lib/monopoly.mjs';
...
export default async function handler(req,res) {
  ...
  try {
    ...
    if(req.method==='GET') {
      ...
      if(action==='monopolyRooms'){assert(verify(req).role==='admin','เฉพาะผู้สอน',403);return send(200,{rooms:await listMonopolies(),serverNow:Date.now()});}
    ...
    if(action==='monopolyCreate'){const actor=verify(req);assert(actor.role==='admin','เฉพาะผู้สอน',403);let g;for(let i=0;i<5;i++){const code=randomBytes(3).toString('hex').toUpperCase();g=newMonopoly(code,Date.now());g.public=monopolyPublic(g);if(await createMonopoly(g))return send(200,{game:normalize(g),serverNow:Date.now()});}throw new GameError('สร้างห้องเกมเศรษฐีไม่สำเร็จ',503);}
    if(action==='monopolyDelete'){const actor=verify(req);assert(actor.role==='admin','เฉพาะผู้สอน',403);assert(await deleteMonopoly(code),'ไม่พบห้องนี้',404);return send(200,{ok:true,serverNow:Date.now()});}
    ...
    if(['monopolyRoll','monopolyAnswer','monopolyContinue','monopolyTick'].includes(action)){const actor=action==='monopolyAnswer'?verify(req,code):{};const result=await updateMonopoly(code,draft=>mutateMonopoly(draft,action,data,actor,Date.now()));return send(200,{...result.result,state:result.game.public,serverNow:Date.now()});}
