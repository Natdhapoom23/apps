...
test('admin saves typed question formats, board length, and hidden state across a reset',()=>{const g=newMonopoly('ABC123',0);mutateMonopoly(g,'monopolySave',{title:'ใหม่',boardLength:24,spaces:Array.from({length:24},()=>({title:'ช่อง'})),questions:[{type:'choice',title:'เลือก',options:['ก','ข','ค','ง'],correct:2,seconds:5,active:false},{type:'image-count',title:'นับจุด',answer:7,seconds:15,image:'https://example.com/x.png',active:true}],teams:[{id:'a',name:'ทีม A'}]},admin,1);assert.equal(g.boardLength,24);assert.equal(g.questionBank.length,2);assert.equal(g.questionBank[1].answer,7);assert.equal(g.questionBank[0].active,false);mutateMonopoly(g,'monopolyReset',{},admin,2);assert.equal(g.questionBank[0].active,false);assert.equal(g.questionBank[1].active,true);});
