...
test('all teams can answer and the fastest correct team gets one bonus space',()=>{const g=game();g.questionBank=[g.questionBank[0]];const green=join(g,'green','team1');const red=join(g,'red','team0');mutateMonopoly(g,'monopolyRoll',{}, {},1);const r=g.lastRoll;mutateMonopoly(g,'monopolyTick',{}, {},3000);assert.equal(r.phase,'question');const before=g.players.team0.position;const slow=mutateMonopoly(g,'monopolyAnswer',{roundId:r.id,choice:0},green,4000);const fast=mutateMonopoly(g,'monopolyAnswer',{roundId:r.id,choice:0},red,3200);assert.ok(slow.answer.score<fast.answer.score);mutateMonopoly(g,'monopolyTick',{}, {},14000);assert.equal(g.lastRoll.phase,'resolved');assert.equal(g.players.team0.position,Math.min(30,before+1));assert.equal(g.lastRoll.hero.name,'red');});
test('wrong answers do not move teams and timeout ends the turn',()=>{const g=game();const player=join(g,'wrong','team0');mutateMonopoly(g,'monopolyRoll',{}, {},1);const r=g.lastRoll;mutateMonopoly(g,'monopolyTick',{}, {},3000);const pos=g.players.team0.position;mutateMonopoly(g,'monopolyAnswer',{roundId:r.id,choice:1},player,3500);mutateMonopoly(g,'monopolyTick',{}, {},14000);assert.equal(g.lastRoll.correct,false);assert.equal(g.players.team0.position,pos);assert.equal(g.turn,1);});
