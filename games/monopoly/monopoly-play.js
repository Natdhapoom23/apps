...
function render(){
 ...
 $('#turn-label').textContent=animating?'กำลังเดิน…':`ตาของ ${current?.name||'—'}`;
 ...
 for(let side=0;side<4;side++){
  ...
  for(let j=0;j<count;j++){
   ...
   e.append(node('span','',index+1),node('span','secret','?'));const tokens=node('div','tokens');
 ...
 $('#board').replaceChildren(...tiles);$('#roll').disabled=busy||animating||!id||current?.id!==id;
}
async function playResult(result){
 ...
 for(let i=1;i<=result.roll;i++){positions[result.player]=(result.from-1+i)%state.spaces.length+1;render();await delay(180);}
 const space=state.spaces[result.spaceId-1];$('#landing-number').textContent=`${result.name} · ช่อง ${result.spaceId}`;$('#landing-title').textContent=space?.title||'เปิดป้าย';$('#landing-text').textContent=result.message;$('#landing-effect').textContent=`${result.to!==result.spaceId?`ย้ายไปช่อง ${result.to} · `:''}คะแนน ${space?.points||0}`;
